const App = () => {
  ...
  .catch(error => console.error('Ocorreu um erro ao obter os filmes de ficção científica:', error));
...
function fetchTodosFilmesFiccaoCientifica() {
  ...
  return fetch(url)
    ...
    }); // essa função é responsável por fazer uma requisição à API do TMDb para obter uma lista de filmes de ficção científica. Ela retorna uma promessa que, quando resolvida, fornecerá os resultados da requisição.
}
...
export { InserirFilmesNaTela, fetchTodosFilmesFiccaoCientifica };
